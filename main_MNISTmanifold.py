...
from utils.utils import sample_grid, vec2map, MnistLoader
from utils.fileIO import creat_folder, get_args, loss_recorder, save_result

task_name = 'MnistManifold/'
size = [28, 28]
...
def train_net(Estimator,
              Classifier,
              args):  
    pathname = '../scratch/MnistManifold_data/'###############NEED TO BE REVISED ACCORDINGLY###########
    ...
    trainset = MnistLoader(pathname, phase = 'train', transform=transform)
    testset = MnistLoader(pathname, phase = 'test', transform=transform)
    ...
    for epoch in range(epochs):
        with tqdm(total=n_train, desc='Epoch {}/{}'.format(epoch, epochs), unit='img') as pbar:
            ...
            running_belc = 0.0
            running_lapl = 0.0
            for count,(IM, label,_) in enumerate(train_loader):###############NEED TO BE REVISED ACCORDINGLY###########
                n_sample = IM.shape[0]
...
if __name__ == '__main__':
    ...
    Classifier = ClassifierMINI(n_input = 3, n_output = 10, size = size)###############NEED TO BE REVISED ACCORDINGLY###########
    if args.unload:
