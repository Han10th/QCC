...
from utils.utils import sample_grid, vec2map, CifarLoader
from utils.fileIO import creat_folder, get_args, loss_recorder, save_result

task_name = 'CifarManifold/'
size = [32, 32]
...
def train_net(Estimator,
              Classifier,
              args):  
    pathname = '../scratch/CifarManifold_data/'###############NEED TO BE REVISED ACCORDINGLY###########
    ...
    trainset = CifarLoader(pathname, phase = 'train', transform=transform)
    testset = CifarLoader(pathname, phase = 'test', transform=transform)
